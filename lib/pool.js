...
const addressBase58Prefix = cnUtil.address_decode(Buffer.alloc(global.config.poolServer.poolAddress))
...
function BlockTemplate (template) {
  ...
  this.buffer = Buffer.alloc(this.blob, 'hex')
...
Miner.prototype = {
  ...
  getTargetHex: function () {
    ...
    const diffBuff = diff1.div(this.difficulty).toBuffer.alloc()
    diffBuff.copy(padded, 32 - diffBuff.length)

    const buff = padded.slice(0, 4)
    const buffArray = buff.toByteArray().reverse()
    const buffReversed = Buffer.alloc(buffArray)
  ...
  getJob: function () {
    ...
    const blockTemplate = Buffer.alloc(currentBlockTemplate.blob, 'hex')
    const rootBlockTemplate = Buffer.alloc(blob, 'hex')
...
function processShare (miner, job, blockTemplate, nonce, resultHash) {
  ...
  const shareBuffer = cnUtil.construct_block_blob(template, Buffer.alloc(nonce, 'hex'))
  ...
  if (shareTrust.enabled && shareTrust.checkTrust(miner.ip, miner.login, job.difficulty)) {
    hash = Buffer.alloc(resultHash, 'hex')
    shareType = 'trusted'
  } else {
  ...
  const hashNum = bignum(Buffer.alloc(hashArray).toString('hex'), 16)
