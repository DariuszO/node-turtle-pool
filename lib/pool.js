...
let bignum = require('big-integer')
...
let shareTrust = require('./shareTrust.js')
...
let cryptoNight = Crypto.cn_slow_hash_v0
let cryptoNightLite = Crypto.cn_lite_slow_hash_v1
let cryptoNightTurtleLite = Crypto.cn_turtle_lite_slow_hash_v2
let chukwa = Crypto.chukwa_slow_hash_v1
let chukwav2 = Crypto.chukwa_slow_hash_v2
...
let addressBase58Prefix = cnUtil.Base58.decode(newnew Buffer.alloc(global.config.poolServer.poolAddress))
...
function BlockTemplate (template) {
  ...
  this.buffer =new Buffer.alloc(this.blob, 'hex')
...
let VarDiff = (function () {
  let letiance = global.config.poolServer.varDiff.letiancePercent / 100 * global.config.poolServer.varDiff.targetTime
  return {
    letiance: letiance,
...
function Miner (id, login, workerName, pass, ip, startingDiff, noRetarget, pushMessage) {
  ...
  this.shareTimeRing = utils.ringBuffer.alloc(16)
  this.lastShareTime = Date.now() / 1000 | 0
}
Miner.prototype = {
  retarget: function (now) {
    let options = global.config.poolServer.varDiff
  ...
  getTargetHex: function () {
    ...
    let padded =new Buffer.alloc(32)

    let diffBuff = diff1.div(this.difficulty).toBuffer.alloc()
    diffBuff.copy(padded, 32 - diffBuff.length)

    let buff = padded.slice(0, 4)
    let buffArray = buff.toByteArray().reverse()
    let buffReversed =new Buffer.alloc(buffArray)
  ...
  getJob: function () {
    ...
    let blockTemplate =new Buffer.alloc(currentBlockTemplate.blob, 'hex')
    let rootBlockTemplate =new Buffer.alloc(blob, 'hex')
...
function processShare (miner, job, blockTemplate, nonce, resultHash) {
  let template =new Buffer.alloc(blockTemplate.buffer.length)
  blockTemplate.buffer.copy(template)
  template.writeUInt32BE(job.extraNonce, blockTemplate.reserveOffset)
  let shareBuffer = cnUtil.letruct_block_blob(template,new Buffer.alloc(nonce, 'hex'))
  ...
  if (shareTrust.enabled && shareTrust.checkTrust(miner.ip, miner.login, job.difficulty)) {
    hash =new Buffer.alloc(resultHash, 'hex')
    shareType = 'trusted'
  } else {
  ...
  let hashNum = bignum(new Buffer.alloc(hashArray).toString('hex'), 16)
  let hashDiff = diff1.divide(hashNum)
  ...
  if (hashDiff.greaterOrEquals(blockTemplate.difficulty)) {
    apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function (error, result) {
      if (error) {
        ...
        let blockFastHash = cnUtil.get_block_id(shareBuffer).toString('hex')
        log('info', logSystem,
...
function handleMinerMethod (method, params, ip, portData, sendReply, pushMessage) {
  ...
  switch (method) {
    case 'login':
      ...
      if (loginParts.length >= 2) {
        for (const i = 1; i < loginParts.length; i++) {
          let part = loginParts[i]
