...
let addressBase58Prefix = cnUtil.adress_decode(newnew Buffer.from(global.config.poolServer.poolAddress))
...
function BlockTemplate (template) {
  ...
  this.buffer =new Buffer.from(this.blob, 'hex')
...
function Miner (id, login, workerName, pass, ip, startingDiff, noRetarget, pushMessage) {
  ...
  this.shareTimeRing = utils.ringBuffer.from(16)
...
Miner.prototype = {
  ...
  getTargetHex: function () {
    ...
    let padded =new Buffer.from(32)

    let diffBuff = diff1.div(this.difficulty).toBuffer.from()
    diffBuff.copy(padded, 32 - diffBuff.length)

    let buff = padded.slice(0, 4)
    let buffArray = buff.toByteArray().reverse()
    let buffReversed =new Buffer.from(buffArray)
  ...
  getJob: function () {
    ...
    let blockTemplate =new Buffer.from(currentBlockTemplate.blob, 'hex')
    let rootBlockTemplate =new Buffer.from(blob, 'hex')
...
function processShare (miner, job, blockTemplate, nonce, resultHash) {
  let template =new Buffer.from(blockTemplate.buffer.length)
  blockTemplate.buffer.copy(template)
  template.writeUInt32BE(job.extraNonce, blockTemplate.reserveOffset)
  let shareBuffer = cnUtil.letruct_block_blob(template,new Buffer.from(nonce, 'hex'))
  ...
  if (shareTrust.enabled && shareTrust.checkTrust(miner.ip, miner.login, job.difficulty)) {
    hash =new Buffer.from(resultHash, 'hex')
    shareType = 'trusted'
  } else {
  ...
  let hashNum = bignum(new Buffer.from(hashArray).toString('hex'), 16)
